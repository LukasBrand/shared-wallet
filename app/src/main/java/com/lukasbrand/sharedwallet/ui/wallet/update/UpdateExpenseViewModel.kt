...
class UpdateExpenseViewModel @Inject constructor(
    ...
    val expense: Expense = savedStateHandle["expense"]!!
    ...
    init {
        viewModelScope.launch {
            name.value = UiState.Set(expense.name)
            _owner.value = UiState.Set(expense.owner)
            _location.value = UiState.Set(expense.location)
            _creationDate.value = UiState.Set(expense.creationDate)
            _dueDate.value = UiState.Set(expense.dueDate)
            expenseAmount.value = UiState.Set(expense.expenseAmount.toPlainString())
            _participants.value = UiState.Set(expense.participants)
        }
    }
    ...
    fun updateExpense() {
        _updateExpenseCompleted.value = Navigator.Loading
        viewModelScope.launch {
            val name = when (val o = name.value) {
                is UiState.Set<String> -> o.data
                UiState.Unset -> {
                    _updateExpenseCompleted.value =
            ...
            try {
                ...
                expensesRepository.modifyExpense(newExpense)
                _updateExpenseCompleted.value = Navigator.Navigate(Unit)
            } catch (exception: Exception) {
                _updateExpenseCompleted.value = Navigator.Error(exception)
            }
        }
    }

    private val _updateExpenseCompleted: MutableStateFlow<Navigator<Unit>> =
        MutableStateFlow(Navigator.Stay)
    val updateExpenseCompleted: StateFlow<Navigator<Unit>> = _updateExpenseCompleted

    fun onNavigatedAfterExpenseUpdated() {
        _updateExpenseCompleted.value = Navigator.Stay
    }
